...
def make_hadamard(shape):
    ...
    for i in range(len(hadamard_shape) -1):
        hadamard = np.r_[hadamard,hadamard]
        hadamard = np.c_[hadamard,hadamard]
...
def make_walsh_hadamard(shape, hadamard_matrix, zero_crossing):
    #copy()　にしないと walsh_hadamard の変更が
    #hadamard_matrix[shape]にも適用されてしまう enocodeは参照するだけなので平気
...
def main():
    ...
    img = cv2.imread("img/sample_64pixel.bmp",0)
    print(img.shape)
